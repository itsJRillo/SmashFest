...
public class PlayerMovement : MonoBehaviourPunCallbacks, IPunObservable {
    ...
    void Start() {
        if(!PhotonNetwork.IsConnected){
            ...
            if (!view.IsMine) {
                ...
                playerStatus.healthBar = canvas.transform.Find("HealthBar_Enemy").GetComponent<Image>();
            }
            else {
                PlayerStatus playerStatus = GetComponent<PlayerStatus>();
    ...
    void Update() {
        if (PhotonNetwork.IsConnected && !view.IsMine)
            return; // Solo permitir el control para el jugador local o en el modo de un solo jugador
        ...
        if (PhotonNetwork.IsConnected && (Vector3)rb.position != lastPosition) {
            view.RPC("UpdatePlayerPosition", RpcTarget.OthersBuffered, rb.position);
